...
{
    public static async Task EnsureDatabaseAndMigrateAsync(string appConnectionString, string _databaseNameIgnored = "")
    {
        // SQLite creates the DB file automatically on connect.
        await RunEmbeddedMigrationsAsync(appConnectionString);
    }
    ...
    private static async Task RunEmbeddedMigrationsAsync(string connString)
    {
        ...
        var scripts = assembly.GetManifestResourceNames()
            .Where(n => n.Contains(".Sql.", StringComparison.OrdinalIgnoreCase) && n.EndsWith(".sql", StringComparison.OrdinalIgnoreCase))
    ...
    {
        // Simple splitter good enough for your current scripts.
        // (No triggers/procs here, just DDL statements.)
        return sql.Split(';')
            .Select(s => s.Trim())
